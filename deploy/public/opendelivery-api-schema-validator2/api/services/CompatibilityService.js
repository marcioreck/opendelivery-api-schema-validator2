...
class CompatibilityService {
    ...
    async checkCompatibility(sourceVersion, targetVersion, payload) {
        try {
            logger_1.logger.info(`Checking compatibility from ${sourceVersion} to ${targetVersion}`);
            ...
            logger_1.logger.info(`Compatibility check completed: ${result.compatible ? 'compatible' : 'incompatible'}`, {
            ...
            logger_1.logger.error('Compatibility check error:', error);
            if (error instanceof errorHandler_1.AppError) {
                throw error;
            }
            throw new errorHandler_1.AppError(500, 'Compatibility service error');
        }
    }
    determineCompatibility(sourceVersion, targetVersion, sourceValidation, targetValidation) {
    ...
    getCompatibilityLevel(sourceVersion, targetVersion, sourceValidation, targetValidation) {
        if (sourceValidation.valid && targetValidation.valid) {
    ...
    analyzeChanges(sourceVersion, targetVersion, sourceValidation, targetValidation) {
        const changes = [];
        ...
        const fixedErrors = sourceErrors.filter((sourceError) => !targetErrors.some((targetError) => targetError.path === sourceError.path && targetError.message === sourceError.message));
        ...
        const newErrors = targetErrors.filter((targetError) => !sourceErrors.some((sourceError) => sourceError.path === targetError.path && sourceError.message === targetError.message));
    ...
    getVersionSpecificChanges(sourceVersion, targetVersion) {
        const changes = [];
    ...
    generateRecommendations(sourceVersion, targetVersion, sourceValidation, targetValidation) {
