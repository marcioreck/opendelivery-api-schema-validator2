...
export default function CompatibilityPage() {
  ...
  // Force versions to be 1.5.0 on component mount
  useEffect(() => {
    if (!sourceVersion || sourceVersion === '') {
      setSourceVersion('1.5.0');
    }
    if (!targetVersion || targetVersion === '') {
      setTargetVersion('1.5.0');
    }
  }, []);
        ...
        <Grid item xs={12} md={4}>
          <Paper sx={{ p: 2, mb: 2 }}>
            ...
            <FormControl fullWidth sx={{ mb: 2 }}>
              ...
              <Select
                key={`source-version-select-${sourceVersion || '1.5.0'}`}
                value={sourceVersion || '1.5.0'}
                defaultValue="1.5.0"
                onChange={(e) => setSourceVersion(e.target.value)}
                label="Versão de Origem"
                displayEmpty={false}
                renderValue={(selected) => selected || '1.5.0'}
              >
                {SCHEMA_VERSIONS.map((version) => (
                  <MenuItem key={version} value={version} selected={version === '1.5.0'}>
                    {version}
                  </MenuItem>
                ))}
              </Select>
            ...
            <FormControl fullWidth sx={{ mb: 2 }}>
              ...
              <Select
                key={`target-version-select-${targetVersion || '1.5.0'}`}
                value={targetVersion || '1.5.0'}
                defaultValue="1.5.0"
